/**
 * @file mekf.c
 * @brief 3-State MEKF Implementation.
 *
 * Reduced from 6-state to 3-state by removing bias estimation.
 *
...
void mekf_init(MEKF_t *f, Quaternion_t initial_q) {
    ...
    // Initial uncertainties for attitude
...
void mekf_predict(MEKF_t *f, Vector3_t omega_meas, float dt) {
    // 1. Direct Gyro Measurement (No Bias Correction)
    Vector3_t w = omega_meas;
    ...
    float Phi[3][3] = {0};
    for(int i = 0; i < 3; i++) Phi[i][i] = 1.0f;
    ...
    for(int i = 0; i < 3; i++) {
        for(int j = 0; j < 3; j++) {
            for(int k = 0; k < 3; k++) tmp[i][j] += Phi[i][k] * f->P[k][j];
        }
    }
    for(int i = 0; i < 3; i++) {
        for(int j = 0; j < 3; j++) {
            float val = 0;
            for(int k = 0; k < 3; k++) val += tmp[i][k] * Phi[j][k];
            f->P[i][j] = val;
        }
...
void mekf_update(MEKF_t *f, Quaternion_t q_meas, float r_noise) {
    ...
    float IKH[3][3] = {0};
    for(int i = 0; i < 3; i++) IKH[i][i] = 1.0f;
    for(int i = 0; i < 3; i++) {
    ...
    for(int i = 0; i < 3; i++) {
        for(int j = 0; j < 3; j++) {
            for(int k = 0; k < 3; k++) tmpP[i][j] += IKH[i][k] * f->P[k][j];
        }
    }
    for(int i = 0; i < 3; i++) {
        for(int j = 0; j < 3; j++) {
            float val = 0;
            for(int k = 0; k < 3; k++) val += tmpP[i][k] * IKH[j][k];
            f->P[i][j] = val;
        }
    }
    for(int i = 0; i < 3; i++) {
