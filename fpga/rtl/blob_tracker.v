...
module blob_tracker #(
    ...
    parameter MIN_PIXEL_VAL = 16'd500, 
    parameter MIN_MASS = 24'd5000,     
    parameter MIN_HITS = 5'd3          
)(
    input  wire                   clk,
    input  wire                   rst_n,
    ...
    // --- Matching Logic with Saturation ---
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            centroid_x <= 0; centroid_y <= 0; // Fixed: Initialized for clean waveforms
        end else begin
            if (frame_done) begin global_flush_mode <= 1'b1; frame_seen <= 1'b1; end
            ...
            if (pixel_v && pixel_i >= MIN_PIXEL_VAL && !global_flush_mode) begin
                if (|match_bus) begin
                    ...
                    slot_px_cnt[empty_idx] <= 5'd1; slot_x_min[empty_idx] <= calc_x_min;
                    slot_x_max[empty_idx] <= calc_x_max; slot_last_y[empty_idx] <= pixel_y;
                end
            end
                ...
                S_IDLE: begin
                    if (slot_active[flush_ptr] && (global_flush_mode || (pixel_y > ((slot_last_y[flush_ptr] >= 10'd1021) ? 10'd1023 : slot_last_y[flush_ptr] + 10'd2)))) begin
                        if (slot_m00[flush_ptr] >= MIN_MASS && slot_px_cnt[flush_ptr] >= MIN_HITS) begin 
                            ...
                            slot_active[flush_ptr] <= 0; slot_m00[flush_ptr] <= 0; slot_m10[flush_ptr] <= 0;
                            slot_m01[flush_ptr] <= 0; slot_px_cnt[flush_ptr] <= 0;
                            slot_x_min[flush_ptr] <= 0; slot_x_max[flush_ptr] <= 0; slot_last_y[flush_ptr] <= 0;
                            flush_ptr <= flush_ptr + 1; if (flush_ptr == 15 && global_flush_mode) global_flush_mode <= 0;
                        end
                    end else begin
                        flush_ptr <= flush_ptr + 1; if (flush_ptr == 15 && global_flush_mode) global_flush_mode <= 0;
                    end
                ...
                S_DIVIDE: begin
                    if (d_cnt < 40) begin
                        if (d_accum >= ({24'b0, d_divisor} << (39-d_cnt))) begin
                            ...
                            d_quot[39-d_cnt] <= 1'b0; // Fixed: Explicit bit clearing
                        end
                        d_cnt <= d_cnt + 1;
                    end else begin
                        if (!d_mode_y) begin
                            ...
                            centroid_y <= d_quot[23:0]; centroid_valid <= 1; state <= S_WAIT_READY;
                        end
                    end
                end
                S_WAIT_READY: if (centroid_valid && centroid_ready) begin
                    centroid_valid <= 0; slot_active[flush_ptr] <= 0;
                    slot_m00[flush_ptr] <= 0; slot_m10[flush_ptr] <= 0; slot_m01[flush_ptr] <= 0;
                    slot_px_cnt[flush_ptr] <= 0; slot_x_min[flush_ptr] <= 0; slot_x_max[flush_ptr] <= 0;
                    slot_last_y[flush_ptr] <= 0; flush_ptr <= flush_ptr + 1;
