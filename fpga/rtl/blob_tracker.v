...
// Description: 16-slot Bounding Box Tracker with Global Flush and AXI-Handshake.
...
module blob_tracker #(
    ...
    input  wire                   pixel_v,
    input  wire                   frame_done,    // Pulse at end of frame (VSYNC)
    ...
    input  wire                   centroid_ready // Backpressure from downstream
);
    ...
    // --- Matching Logic (Pipelined/Simplified for Timing) ---
    reg [MAX_BLOBS-1:0] match_bus;
    integer i;
    always @(*) begin
        for (i = 0; i < MAX_BLOBS; i = i + 1) begin
            // Simple magnitude comparison (Fast in LUTs)
    ...
    // Priority Encoder to find first match or first empty
    wire [3:0] match_idx, empty_idx;
    wire       match_found, empty_found;
    
    assign match_found = |match_bus;
    // Simple priority encoders (synthesize to small LUT trees)
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // 2. Pixel Ingestion (Highest Priority for Slot RAM)
            if (pixel_v && pixel_i > 0 && !global_flush_mode) begin
                if (match_found) begin
                    ...
                    slot_x_min[match_idx]   <= (pixel_x > 3) ? pixel_x - 3 : 0;
                    slot_x_max[match_idx]   <= pixel_x + 3;
                    slot_y_limit[match_idx] <= pixel_y + 1;
                    ...
                    slot_x_min[empty_idx]   <= (pixel_x > 3) ? pixel_x - 3 : 0;
                    slot_x_max[empty_idx]   <= pixel_x + 3;
                    slot_y_limit[empty_idx] <= pixel_y + 1;
                end
            end

            // 3. State Machine (Flush & Divide)
            case (state)
                S_IDLE: begin
                    centroid_valid <= 0;
                    // Check if current flush_ptr needs processing
                ...
                S_DIVIDE: begin
                    if (d_cnt < 40) begin
                        // Sequential Radix-2
                        if (d_accum >= (d_divisor << (39-d_cnt))) begin
                            d_accum <= d_accum - (d_divisor << (39-d_cnt));
                            d_quot[39-d_cnt] <= 1;
                        end else d_quot[39-d_cnt] <= 0;
                        d_cnt <= d_cnt + 1;
                    end else begin
                        if (!d_mode_y) begin // Finished X
                            centroid_x <= d_quot[23:0];
                            d_mode_y   <= 1;
                            d_cnt      <= 0;
                            d_accum    <= {slot_m01[flush_ptr], 8'b0};
                        end else begin // Finished Y
                ...
                S_WAIT_READY: begin
                    centroid_valid <= 1'b1;
                    if (centroid_ready) begin
                        centroid_valid <= 1'b0;
                        slot_active[flush_ptr] <= 1'b0; // Clear slot
                        state <= S_IDLE;
                    end
                end
            endcase
