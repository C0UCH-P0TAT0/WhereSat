...
module vision_pipeline_top #(
    ...
    line_buffer #(.DATA_WIDTH(DATA_WIDTH), .IMAGE_WIDTH(IMAGE_WIDTH)) u_line_buffer (
        .clk(clk), .rst_n(rst_n), .pixel_in(pixel_in), .valid_in(valid_in),
    ...
    threshold_filter #(.DATA_WIDTH(DATA_WIDTH), .IMAGE_WIDTH(IMAGE_WIDTH)) u_threshold_filter (
        .clk(clk), .rst_n(rst_n), .window_data(window_data), .valid_in(window_valid),
        .dynamic_threshold(THRESHOLD), .f_pixel(f_pixel), .f_x(f_x), .f_y(f_y), .f_valid(f_valid)
    );

    blob_tracker #(.DATA_WIDTH(DATA_WIDTH), .MAX_BLOBS(16)) u_blob_tracker (
        .clk(clk), .rst_n(rst_n), .pixel_i(f_pixel), .pixel_x(f_x), .pixel_y(f_y), .pixel_v(f_valid),
        .frame_done(frame_done), .centroid_x(centroid_x), .centroid_y(centroid_y),
        .centroid_valid(centroid_valid), .centroid_ready(centroid_ready), .processing_done(processing_done)
    );
