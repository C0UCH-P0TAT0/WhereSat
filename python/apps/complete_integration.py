...
from core.star_id import identify_stars, calculate_triangle_fingerprint, pixels_to_vectors
...
class HILSimulation(QtCore.QThread):
    ...
    def __init__(self):
        ...
        self.q = np.array([-0.805684, -0.290840, -0.075647, -0.510453])  # Initial quaternion (identity)
...
class WhereSatGUI(QtWidgets.QMainWindow):
    ...
    def update_ui(self, data):
        ...
        q_target = np.array([-0.805684, -0.290840, -0.075647, -0.510453])
        
        q_true  = data['q_true']
        q_quest = data['q_quest']
        q_est   = data['q_est']

        # Dot Products (used abs because q and -q are identical attitudes)
