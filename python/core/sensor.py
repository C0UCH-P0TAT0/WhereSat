...
# =========================================================
# THE VIVADO HARDWARE BRIDGE
# =========================================================

def export_frame_to_mem(dirty_image_array: np.ndarray, filename: str = "fpga/tb/tb_frame.mem"):
    """
...
def apply_sensor_dirt(clean_image: np.ndarray, readout_sigma: float, hot_pixel_fraction: float) -> np.ndarray:
    ...
    """ 
    # Convert to float for math operations to prevent overflow/underflow
    ...
    # 2. Hot Pixels (Saturate at 16-bit max, not 8-bit)
    ...
    if num_hot_pixels > 0:
        ...
        noisy_image[y_coords, x_coords] = 65535.0  # 16-bit ceiling
    ...
    # Call the exporter 
    export_frame_to_mem(final_uint16_image, filename="fpga/tb/tb_frame.mem")
